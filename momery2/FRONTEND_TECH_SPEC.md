...
| 类别 | 选型 | 版本 | 选型理由 |
|------|------|------|---------|
| 核心框架 | Vue 3 (Composition API) | ^3.4 | `<script setup>` 语法简洁，组合式 API 利于逻辑复用 |
| 类型系统 | TypeScript | ^5.4 | 全链路类型安全，减少运行时错误 |
| 构建工具 | Vite | ^5 | 极速 HMR，原生 ESM，Vue 官方推荐 |
| 路由 | Vue Router | ^4 | Vue 3 官方路由方案，支持嵌套路由、路由守卫 |
| 状态管理 | Pinia | ^2 | Vue 3 官方推荐，TypeScript 友好，模块化设计 |
| HTTP 客户端 | Axios | ^1 | 拦截器机制适合统一错误处理与数据解包 |
| Markdown 渲染 | marked | ^9 | 轻量高效，支持 GFM 扩展 |
| 代码高亮 | highlight.js | ^11 | 配合 marked 实现卡片内代码块高亮 |
| 富文本编辑器 | Tiptap | ^2 | 基于 ProseMirror，扩展性强，输出 JSON 便于存储 |
| CSS 方案 | Scoped CSS + CSS Variables | — | Vue SFC 原生支持，无额外依赖，主题变量全局注入 |
| 图标方案 | 内联 SVG 组件 | — | 零外部依赖，按需引用，Tree-shaking 友好 |

### 不引入的依赖（与理由）

| 不引入 | 理由 |
|--------|------|
| UI 组件库 (Element Plus / Naive UI 等) | 项目风格为简约办公风，组件全部自建以保持设计一致性 |
| Tailwind CSS / UnoCSS | 项目规模可控，CSS 变量 + Scoped CSS 足够；引入原子化框架会增加学习成本和构建复杂度 |
| 图表库 (ECharts / Chart.js) | 统计模块采用日历热力图 + 数据面板（CSS 自绘），不涉及柱状图/折线图 |
| zod / joi | 参数校验在后端 controller 层用简单条件判断完成 |
...
App.vue
├── LayoutSidebar.vue
│   Props: (无 — 全局布局组件不需要外部传参)
│   State: activeNav (当前选中导航), filterCategory (当前分类筛选)
│   Children:
│   ├── SearchBar.vue
│   │   Events: @search(query: string) — 触发全局搜索
│   └── CategoryFilter.vue
...
│   State: activeTab (当前激活子标签)
│   Slots: #actions (右侧操作区插槽)
...
CardsHome.vue
├── DeckGrid.vue
│   Props: decks: Deck[], loading: boolean
│   Events: @deck-click(deckId: number), @deck-delete(deckId: number)
│   └── DeckCard.vue (v-for)
│       Props: deck: Deck
...
DeckDetail.vue
├── CardList.vue
│   Props: cards: Card[], deckName: string
│   Events: @edit(cardId: number), @delete(cardId: number)
└── CardCreateForm.vue
    Props: visible: boolean, templates: Template[], editCard?: Card
    Events: @submit(data: CardFormData), @cancel
    State: front (string), back (string), previewMode ('edit' | 'preview' | 'split')
    └── MarkdownPreview.vue (内联子组件 — 渲染 marked 输出)

ReviewSession.vue
├── ReviewProgress.vue (进度条 + 剩余卡片数)
└── ReviewCard.vue
    Props: card: Card, currentIndex: number, totalCount: number
    State: showAnswer: boolean
    Events: @rate(result: 'forgot' | 'hard' | 'good' | 'easy')
...
CardStats.vue
├── StatsSummary.vue
│   Props: stats: StatsOverview
│   — 展示总卡片数 / 已掌握 / 待复习 / 今日待复习 / 正确率 / 连续天数
├── CalendarHeatmap.vue
│   Props: data: CalendarDay[], year: number, month: number
│   Events: @day-click(date: string)
│   — 月历网格，每格按 reviewCount 着色（0-4 层深浅）
└── TimelineView.vue
    Props: items: TimelineItem[]
    Events: @item-click(item: TimelineItem)
    — 纵向时间线，展示笔记/卡片创建 + 复习记录
```
...
NotesList.vue
├── FolderTree.vue
│   Props: tree: FolderNode[], selectedFolder: number | null
│   Events: @folder-select(id: number), @folder-create(name: string), @folder-delete(id: number)
└── NoteCard.vue (v-for)
    Props: note: Note, folderName: string
    Events: @click, @move-to-folder(noteId, folderId)
...
NoteEditor.vue
    Props: noteId: number | null (null = 新建)
    State: title, content (Tiptap JSON), isToday, tags
    Events: @save, @convert-to-cards
    └── TiptapEditor.vue
        Props: modelValue: object, editable: boolean
...
```
Modal.vue
    Props: visible: boolean, title: string, width: string, closable: boolean
    Slots: #default (内容), #footer (底部按钮)
    Events: @close

ImportModal.vue
    State: file: File | null, preview: ImportPreview | null, loading: boolean
    Events: @import(data)

ExportModal.vue
    Props: decks: Deck[]
    Events: @export(deckId: number | 'all')

TemplateModal.vue
    Props: templates: Template[]
    Events: @select(template: Template)
```
...
| path | name | component | meta.title | meta.tabGroup | 说明 |
|------|------|-----------|------------|---------------|------|
| `/` | home | DashboardHome | 首页 | — | 首页总览仪表盘 |
| `/cards` | cards | CardsHome | 记忆卡片 | cards | 牌组网格页 |
| `/cards/stats` | card-stats | CardStats | 卡片统计 | cards | 统计视图页 |
| `/cards/exam` | exam-goals | ExamGoals | 考试与目标 | cards | 考试目标页 |
| `/cards/deck/:id` | deck-detail | DeckDetail | 牌组详情 | — | 牌组内卡片列表 |
| `/cards/review/:deckId` | review | ReviewSession | 复习 | — | 复习模式全屏 |
| `/notes` | notes | NotesList | 文档 | notes | 文件夹+笔记列表 |
| `/notes/today` | today-notes | TodayNotes | 今日笔记 | notes | 今日临时笔记 |
| `/notes/:id` | note-editor | NoteEditor | 编辑笔记 | — | 笔记编辑器 |
| `/search` | search | SearchView | 搜索 | — | 全局搜索结果 |
| `/settings` | settings | Settings | 设置 | — | 个性化设置 |
...
| `beforeEach` (全局) | 权限校验占位 | 直接 `next()`，Phase 2 启用认证检查 |
...
actions:
  - fetchDecks(params?: { search?, category? })   → GET /api/decks
...
state:
  ...
  - sessionStats: { reviewed, correct, hard, easy }

actions:
  - fetchDueCards(deckId)   → GET /api/decks/:deckId/due
  - submitReview(cardId, result) → POST /api/cards/:cardId/review
...
- Phase 1：无额外 header
- Phase 2：注入 `Authorization: Bearer <token>`
...
DTO 类型：     CreateDeckDto, UpdateDeckDto, CreateCardDto, ReviewResult...
API 响应类型：  ApiResponse<T>, PaginatedData<T>
